#ifndef lint	/* .../appleprint/dwb/text/grap.d/label.c */
#define _AC_NAME label_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.2 87/11/11 21:53:22}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987	Version 1.2 of label.c on 87/11/11 21:53:22";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	Attr *ap;
{
	...
	if (!ap->op) {	/* no explicit size command */
		...
		case '-':	/* better be only one digit! */
			sprintf(buf, "\\s%c%d%s\\s0", ap->op, n, ap->sval);
			break;
		case '*':
		case '/':
