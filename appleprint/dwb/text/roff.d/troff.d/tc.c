#ifndef lint	/* .../appleprint/dwb/text/roff.d/troff.d/tc.c */
#define _AC_NAME tc_c
#define _AC_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.2 87/11/11 22:07:59}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987	Version 1.2 of tc.c on 87/11/11 22:07:59";
...
char *argv[];
{
	...
	setbuf(stdout, buf);
	while ( (ch = getopt(argc, argv, "T:a:eo:d:ct#")) != EOF) {
		switch (ch) {
		case 'T':
			if (strcmp(optarg, "cat") == 0) {	/* use the old one */
				if (fork() == 0) {
	...
	if (sharpsign == 1) {
		fprintf(stderr,"aspect = %g, erase = %d, dbg = %d, keepon = %d\n",
			aspect, erase, dbg, keepon);
