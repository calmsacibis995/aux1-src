#ifndef lint	/* .../appleprint/dwb/text/roff.d/troff.d/devi10/buildrast.c */
#define _AC_NAME buildrast_c
#define _AC_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.2 87/11/11 22:03:40}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987	Version 1.2 of buildrast.c on 87/11/11 22:03:40";
 ...
 */
	...
	if ( (fontoutput = wantfont(name)) == ON )
...
using(size)
    ...
    int		size;			/* building things for this size */
 ...
 */
    ...
    if ( (fd = open(buff, O_WRONLY | O_CREAT, 0644)) == -1 )
 ...
 */
    ...
    if ( fontoutput == OFF  ||  size != unitwidth )
	return;
	...
	if ( (chwidth = charinfo[i].chwidth) < 0 )  {
	    ...
	    chwidth = PIXEL_WIDTH(chwidth, res);
	}    /* End if */
 ...
 * don't find the character or it hasn't been set we'll use unitwidth and
 * res to pick reasonable values.
 *
 */
