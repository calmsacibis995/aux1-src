#ifndef lint	/* .../appleprint/dwb/text/roff.d/troff.d/devi10/oldrast.c */
#define _AC_NAME oldrast_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.2 87/11/11 22:05:44}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987	Version 1.2 of oldrast.c on 87/11/11 22:05:44";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	int s;
{
	...
	if (fontdata[i].fsp[j] == NULL) {
		...
		if (fd == -1)
			error(FATAL, "can't open %s", name);
		if ((n = read(fd, &fh, sizeof(struct Fontheader)))
			!= sizeof(struct Fontheader))
				error(FATAL, "bad raster file %s", name);
		if ((n = read(fd, fs->chp, 256*sizeof(struct Charparam))) <= 0)
				error(FATAL, "bad raster file %s", name);
