#ifndef lint	/* .../appleprint/dwb/text/pic.d/textgen.c */
#define _AC_NAME textgen_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.2 87/11/11 21:57:46}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.2 of textgen.c on 87/11/11 21:57:46";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
obj *plotgen(op, val)	/* plot expr "..." at point */
	obj *op;
	double val;
{
	int i;
	...
	for (i = 0; i < nattr; i++) {
		ap = &attr[i];
		if (ap->a_type == TEXTATTR && ap->a_val.p != NULL) {
			sprintf(buf, ap->a_val.p, val);
			free(ap->a_val.p);
			p = ap->a_val.p = tostring(buf);
			break;
		}
	}
	if (i >= nattr) {	/* no format string */
		sprintf(buf, "%g", val);
		maketattr(CENTER, p = tostring(buf));
	}
	dprintf("plotgen %g in %s\n", val, p);
	return textgen(op);
}
