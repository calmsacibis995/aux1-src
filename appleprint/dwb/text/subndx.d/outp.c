#ifndef lint	/* .../appleprint/dwb/text/subndx.d/outp.c */
#define _AC_NAME outp_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.2 87/11/11 22:12:13}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987	Version 1.2 of outp.c on 87/11/11 22:12:13";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
outp()
{
			...
			if (f) {
				...
				if ((sentp + 1)->cc == BE) {
					if (sentp->leng == 5 && *(sentp->sp) == 't' && *((sentp->sp) + 3) == 'r') {
						index = 14;
						lexp = 1;
					} else if (sentp->leng == 2 && *(sentp->sp) == 'i' ) {
						index = 14;
						lexp = 1;
					}
				}
	...
	if (pstyle || 
	    (sstyle && nn < sthresh) || 
	    (penflg && lexp && nom) || (pepflg && lexp && lpas) || 
	    (pnpflg && nom && lpas) || (penpflg && lpas && nom && lexp) || 
