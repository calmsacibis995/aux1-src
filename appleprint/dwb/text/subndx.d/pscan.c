#ifndef lint	/* .../appleprint/dwb/text/subndx.d/pscan.c */
#define _AC_NAME pscan_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.2 87/11/11 22:12:27}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987	Version 1.2 of pscan.c on 87/11/11 22:12:27";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
char	ce;
{
	...
	if (command == 1)
		...
		subty = 0;
	if (sent[i].cc == ADJ_ADV && sent[i+1].cc != NOUN && sent[i+1].cc != UNK
	     && sent[i+1].cc != ADJ && sent[i+1].cc != NOUN_ADJ)
		sent[i++].cc = ADV;
	done = 0;
	verb = 0;
	verb = getv(i, ce);	/*get verb if obvious*/
	if (command == 0)
		j = getnoun(i, ce);		/*get subject*/
	else 
		j = si;
...
sw:
		...
		case ED:
			...
			continue;
		default:	
			printf("got a %c %o on %sat %d\n", sent[i].cc, sent[i].cc, sent[i].sp, i);
		}
	}
sdone:
	if (sent[i].cc == END && sent[i].ic != ';')
		return(-1);
	else {
		if (sent[i].ic == ';')
			sent[i].cc = ';';
