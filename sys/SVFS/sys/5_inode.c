...
#define _AC_NO_MAIN "@(#) Copyright (c) 1983-87 Sun Microsystems Inc., 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.3 87/11/11 21:32:13}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.3 of 5_inode.c on 87/11/11 21:32:13";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
loop:
	...
	ip->i_fs = fs;
	ip->i_lastr = 0;
	bp = bread(ip->i_devvp, (daddr_t)FsLTOP(ip->i_fs, FsITOD(ip->i_fs, ino)), (int)FsBSIZE(ip->i_fs));
	/*
	 * Check I/O errors
	 */
	...
	int waitfor;
{
	...
	if ((ip->i_flag & (IUPD|IACC|ICHG)) != 0) {
		if (fp->s_ronly)
			return;
		bp = bread(ip->i_devvp,
		    (daddr_t) FsLTOP(ip->i_fs, FsITOD(ip->i_fs, ip->i_number)),
			FsBSIZE(ip->i_fs));
		if (bp->b_flags & B_ERROR) {
			brelse(bp);
			return;
		}
		dp = bp->b_un.b_dino + FsITOO(ip->i_fs, ip->i_number);
		ipack(dp, ip);
