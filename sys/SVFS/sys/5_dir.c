...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1983-87 Sun Microsystems Inc., 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.7 87/11/11 21:31:51}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.7 of 5_dir.c on 87/11/11 21:31:51";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	register struct inode *tdp;		/* parent directory */
{
	...
	if (bmap(sip, (daddr_t)0, B_WRITE,
	     FsBLKOFF(tdp->i_fs, sizeof(struct svfsdirect) * 2), 0) <= 0 ||
	    u.u_error) {
		if (u.u_error == 0)
			u.u_error = ENOSPC;
		return (u.u_error);
