...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.3 87/11/11 21:14:28}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.3 of line.c on 87/11/11 21:14:28";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
register mblk_t *m;
{
	...
	case M_EXDATA:
		...
		if (q->q_count >= TTXOHI*4) {
			if (tp->t_lflag&IXOFF && (tp->t_state&TBLOCK) == 0) {
		...
		if (q->q_count < TTXOLO*4 && tp->t_lflag&IXOFF &&
					     tp->t_state&TBLOCK) {
			m_out = allocb(sizeof(struct iocblk), BPRI_HI);
			if (m_out) {
				m_out->b_cont = allocb(1, BPRI_HI);
		...
		if ((tp->t_lflag&ICANON) == 0) {
			if (q->q_count == 0 && canput(q->q_next)) {
				putnext(q, m);
			} else {
				putq(q, m);
			}
