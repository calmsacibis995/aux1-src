...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.4 87/11/11 21:14:54}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.4 of sxt.c on 87/11/11 21:14:54";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
{
#ifdef NEW_PMMU	
	availsmem -=  btop(sxt_cnt*LINKSIZE);
	availrmem -= btop(sxt_cnt*LINKSIZE);
	...
	if ((sxtbuf = (char *)sptalloc((int)btop(sxt_cnt*LINKSIZE),
				PG_V|PG_RW, -1)) == NULL)
		printf("sxt cannot allocate link buffers\n");
#else
	if ((sxtbuf = (char *)sptalloc((int)btop(sxt_cnt*LINKSIZE),
