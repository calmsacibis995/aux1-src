...
#define _AC_NO_MAIN "@(#) Copyright (c) 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 1.5 87/11/13 12:13:49}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.3 of vfs_syscalls.c on 87/07/18 18:10:23";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	} *uap;
{
	...
	up = &u;
	up->u_error = chdirec(uap->dirnamep, &vp);
	if (up->u_error == 0) {
		VN_RELE(up->u_cdir);
		up->u_cdir = vp;
	}
	...
	} *uap;
{
	...
	if (vattr.va_type == VDIR) {
		vattr.va_mode = (uap->fmode & 0777) & ~up->u_cmask;
	}
	else
		vattr.va_mode = (uap->fmode & 07777) & ~up->u_cmask;
	switch ((int) vattr.va_type) {
	case VBAD:
	case VCHR:
	case VBLK:
	...
	up->u_error = vn_create(uap->pnamep, UIOSEG_USER, &vattr, EXCL, 0, &vp);
	if (up->u_error == 0)
		VN_RELE(vp);
}
	...
	} *uap;
{
	...
	vattr_null(&vattr);
	vattr.va_type = VDIR;
	vattr.va_mode = (uap->dmode & 0777) & ~up->u_cmask;

	up->u_error = vn_create(uap->dirnamep, UIOSEG_USER, &vattr, EXCL, 0, &vp);
	if (up->u_error == 0)
		VN_RELE(vp);
}
	...
	} *uap;
{
	...
	up = &u;
	up->u_error = getvnodefp(uap->fd, &fp);
	if (up->u_error)
		return;
	...
	up->u_error = VOP_READDIR((struct vnode *)fp->f_data, &auio, fp->f_cred);
	if (up->u_error)
		return;
	up->u_error =
	...
	} *uap;
{
	...
	vattr_null(&vattr);
	if (uap->tptr) {
		up->u_error = copyin((caddr_t)uap->tptr, (caddr_t)tv, sizeof (tv));
		if (up->u_error)
			return;
	...
	up->u_error = namesetattr(uap->fname, FOLLOW_LINK, &vattr);
}
	...
	} *uap;
{
	...
	up->u_error = getvnodefp(uap->fd, &fp);
	if (up->u_error == 0)
		up->u_error = VOP_FSYNC((struct vnode *)fp->f_data, fp->f_cred);
}
