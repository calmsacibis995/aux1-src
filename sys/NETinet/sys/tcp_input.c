...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 1.7 87/11/11 21:20:10}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.7 of tcp_input.c on 87/11/11 21:20:10";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
trimthenstep6:
		...
		if (ti->ti_len > tp->rcv_wnd) {
			...
			m_adj(m, -todrop);
#else
			/* XXX work around m_adj bug (fixed in Sun 3.4) */
			if (m->m_len) {
				m_adj(m, -todrop);
			} else {
...
in_localaddr(in)
	struct in_addr in;
{
#ifdef SUBNETS_NOT_LOCAL
	extern u_long subnet_mask;
#endif
	register u_long i = ntohl(in.s_addr);
	register struct ifnet *ifp;
	register struct sockaddr_in *sin;
	register u_long mask;
	static u_long classmap[4] = {
		0xff000000,	/* class A */
		0xff000000,	/* class A */
		0xffff0000,	/* class B */
		0xffffff00,	/* class C */
	...
	/* XXX This horrible hack works on Suns */
#ifdef SUBNETS_NOT_LOCAL
	if ((mask = subnet_mask) == 0)
#endif
		mask = classmap[i >> 30];
