...
#define _AC_NO_MAIN "@(#) Copyright (c) 1983-87 Sun Microsystems Inc., All Rights Reserved.  {Apple version 1.3 87/11/13 12:20:57}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.1 of nfs_vnodeops.c on 87/05/04 01:34:09";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	enum uio_rw rw;
{
	...
	do {
		...
		if (rw == UIO_READ) {
			 ...
			 */
			if (rp->r_size < uio->uio_offset + uio->uio_resid) {
				rp->r_size = uio->uio_offset + uio->uio_resid;
			}
			if (n + on == size) {
				bp->b_resid = 0;
				bp->b_flags |= B_AGE;
				...
				int bytes_in_buf, bytes_in_file;

				/*
				 * b_resid is the number of bytes in the
				 * buffer that are NOT part of the file.
				 * Make sure that we know about all the
				 * bytes we just wrote.
				 */
				bytes_in_buf = n + on;
				if (bp->b_resid > size - bytes_in_buf) {
					bp->b_resid = size - bytes_in_buf;
				}
				/*
				 * But also make sure that if there are
				 * any extra bytes in the buffer that
				 * don't belong to the file that they
				 * are not counted.
				 */
				bytes_in_file = rp->r_size - bn * size;
				if (bp->b_resid < size - bytes_in_file) {
					bp->b_resid = size - bytes_in_file;
				}
				rp->r_flags |= RDIRTY;
				bdwrite(bp);
			}
		}
	} while (u.u_error == 0 && uio->uio_resid > 0 && bp->b_resid == 0);
	...
	register struct buf *bp;
{
	...
	if ((bp->b_flags & B_READ) == B_READ) {
		bp->b_error = nfsread(bp->b_vp, bp->b_un.b_addr,
			...
			(int *)&bp->b_resid, vtor(bp->b_vp)->r_cred);
	} else {
		register struct rnode *rp;
