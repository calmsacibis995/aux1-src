...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.9 87/11/11 21:26:42}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.9 of sig.c on 87/11/11 21:26:42";
#endif		/* _AC_HISTORY */
#endif		/* lint */
	...
	register sig;
{
	...
	case SIGSTOP:
		...
		switch (p->p_stat) {
		case SSLEEP:
			if (action == SIG_DFL && p->p_pri > PZERO) {
...
psig(arg)
{
	...
	if (action != SIG_DFL) {
		if (action == SIG_IGN || rp->p_sigmask & mask)
			panic("psig action");
		up->u_error = 0;
		/*
...
procxmt()
{
	...
	case 4:{
		...
		if(i >= 0){
			i = btotp((u_int)ipcp->ip_addr - (u_int)prp->p_regva);
