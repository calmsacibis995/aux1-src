...
#define _AC_NO_MAIN "@(#) Copyright (c) 1984-85 AT&T-IS, All Rights Reserved.  {Apple version 1.4 87/11/11 21:27:02}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.4 of swapalloc.c on 87/11/11 21:27:02";
#endif		/* _AC_HISTORY */
#endif		/* lint */
				...
				/* set and return an error otherwise. */
{
	...
	*/
	...
	TRACE(T_swapalloc,
	      ("swalloc:size=0x%x swappg=0x%x st_ucnt=0x%x st_nfpgs=0x%x\n", 
		size, (dbd->dbd_blkno - swaptab[smi].st_swplo) >> DPPSHFT, 
		*cntptr, swaptab[smi].st_nfpgs));
			...
			/* block to be removed.			*/
{
	...
	TRACE(T_swapalloc,
	      ("swfree1: dbd=0x%x pgnbr=0x%x st_ucnt=0x%x st_nfpgs=0x%x\n", 
		dbd, pgnbr, *cntptr, st->st_nfpgs));
...
int		nblks;		/* Nbr of blocks to use.	*/
{
	...
	err = (*bdevsw[major(dev)].d_open)(passmajor(dev), 1);
	if(err) {
		st->st_ucnt = NULL;
		st->st_flags = 0;
		return(err);
...
TRACE(T_swapalloc,("st_ucnt=0x%x\n", st->st_ucnt));
#else
	st->st_ucnt = (use_t *)0;
#endif
	...
	for(rp = ractive.r_forw; rp != &ractive; rp = nrp) {
		...
		*/
		...
		for(i = 0  ;  i < seglim  ;  i++){
			...
			pt = rp->r_list[i];
			dbd = dbdget(pt);
			if(rp->r_pgsz - stopg(i) < NPGPT)
				pglim = pt + (rp->r_pgsz - stopg(i));
			else
				pglim = pt + NPGPT;
