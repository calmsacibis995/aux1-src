...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.4 87/11/11 21:28:46}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.4 of meminit.c on 87/11/11 21:28:46";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
int len;
{
	...
	TRACE(T_meminit, ("probing memory at 0x%x: ", start));
	...
	if ( ! setjmp(jb)) {
		...
		*start = 0x61;
		...
		while (max == (caddr_t) -1 || cp < max) {
			...
			if (cp[1] != 0x61 || ! *start) {
				TRACE(T_meminit,("wraparound or write failed @ 0x%x\n",
						cp));
				break;
			}
			current = cp += ptob(1);
	...
	else { TRACE(T_meminit, ("setjmp returned @ 0x%x\n", current)); }
