...
#define _AC_NO_MAIN "@(#) Copyright (c) 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.2 87/11/11 21:28:05}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.2 of ulocking.c on 87/11/11 21:28:05";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
locking() 
{
	...
	if (fp->f_type != DTYPE_VNODE)
		...
		if (((struct vnode *) fp->f_data)->v_op != &svfs_vnodeops)
			return (EREMOTE);
	ip = ((struct inode *) ((struct vnode *) fp->f_data)->v_data);
	 ...
	 * process
	 */
	u.u_error = locked(uap->flag, ip, LB, UB);
	if (u.u_error)
		return;
