...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., All Rights Reserved.  {Apple version 1.11 87/12/07 11:21:45}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.11 of if_ae6.c on 87/12/07 11:21:45";
#endif		/* _AC_HISTORY */
#endif		/* lint */
    ...
    struct args *args;
{
	...
	if (isr & ISR_RXE) {
		...
		printf("ae6int: Rcv overflow, lost %d packets\n", 
		   rp->ae6_cntr2); 
	    }
	}
    }
...
#define	ae6dataaddr(rcvp, off, type)	((type)((caddr_t)((rcvp)+1)+(off)))
	...
	*(mtod(m, struct ifnet **)) = &ae6p->ae6_if;
...
#ifdef ETHERLINK
	{
		...
		redst.sa_data[i] = rcvp->rph_dest.ether_addr_octet[i];
    ...
    u_char *base;
{
	...
	bufp = mtod(m, u16 *);
	if (((unsigned)cp + m->m_len) >= 
	    ((unsigned) base + (unsigned) ((RECV_STOP + 1) * AE6_PAGESIZE))) {
	    templen = ((unsigned) base + 
...
/*
 * This should be moved to raw_ether.c if we want to continue supporting
 * this idea.
 */
ren_output(m0, so)
    struct mbuf *m0;
...
#ifdef NIT
    case AF_NIT:
	if ((ifa = nit_ifwithaddr(&rp->rcb_laddr)) == 0) {
	    error = EADDRNOTAVAIL;
	    goto ren_bad;
	}
	break;
