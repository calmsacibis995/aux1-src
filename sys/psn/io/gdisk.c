...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer Inc., All Rights Reserved.  {Apple version 1.9 87/11/19 18:01:27}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.9 of gdisk.c on 87/11/19 18:01:27";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
struct gentask *taskp;
{
	...
	if(ctp->ctstate == ERRSTATE) {
		...
		case GDR_FAILED:
			GDPRINTF("generic disk c%dd%ds%d Error: ", ctp->ctmajor&7,
	...
	if(ctp->ctretry > MAXRETRY) {
		...
		ctp->ctretry = 0;
		errnow = 1;
		GDPRINTF("generic disk c%dd%ds%d retry limit exceeded: ", ctp->ctmajor&7,
			drp->drnum, drp->drpartnum);
	}
...
register struct gdctl *ctp;
{
	...
	TRACE(T_gdisk, ("in gdstart major = %d ctp = 0x%x\n", 
						ctp->ctmajor, ctp));
	if(!ctp->ctactive) {
...
loop:
	...
	if(bp == ctp->ctbp) {
		TRACE(T_gdisk, ("gdstart: command buffer\n"));
		gdcmd(ctp);
		return;
	}
	switch(ctp->ctstate) {
	...
	if(dp->b_active == 0) {	/* first time for this block */
		TRACE(T_gdisk, ("gdstart pick up new block bp 0x%x bn %d\n", 
			bp, bp->b_blkno));
		bp->b_resid = bp->b_bcount;
		if(bp->b_bcount & (DEV_BSIZE-1)) { /* transfer whole blocks */
			bp->b_error = EIO;
			bp->b_flags |= B_ERROR;
			goto blkdone;
		}
...
blkdone:
	...
	tskp->gtaddr = bp->b_un.b_addr + drp->drcount;
	tskp->gtretproc = gdrw_ret;
	TRACE(T_gdisk, ("gdstart block = %d, addr = 0x%x ctp = 0x%x\n",
		tskp->gtblock, tskp->gtaddr, tskp->gtcp));
