...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., All Rights Reserved.  {Apple version 1.6 87/11/19 18:01:38}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.2 of gdisksubr.c on 87/06/17 15:45:16";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
#endif	STANDALONE
	GDPRINTF("Disk c%dd%ds%d Error: ", ctp->ctmajor&7,
...
again:
	TRACE(T_sdisk, ("in gddriveinit ctp->ctstate = 0x%x\n", ctp->ctstate));
 	drp = ctp->ctactive;
	tskp = ctp->cttaskp;
	tskp->gtaddr = ctp->CTBUF;
	tskp->gtcp = ctp;
...
again:
	...
	case PTS_REINIT:
		if(drp->drpartnum == 31) {
			...
			ptp->ptpsize = ptp->ptlsize = drp->drqual->dqmaxbn;
			ptp->ptoffset = 0;
			ptp->ptdpme = 0;
			ptp->ptstate = PTS_NORMAL;
		...
		if((ptp->ptflags & PTF_NMMASK) == PTF_NMNONE) {
			if((unsigned)drp->drpartnum <= USR_PART)
				strncpy(ptp->pttype, GD_AUX_PTYPE, 
						sizeof(ptp->pttype));
			else
				goto errs;
		}
			...
			if(dpme->dpme_signature == DPME_SIGNATURE 
			   ...
			   && strncmp(dpme->dpme_dpident.dpitype, 
				...
				if(((ptp->ptflags & PTF_NMMASK) == PTF_NMNONE
				  ...
				  ptp->ptname, sizeof(ptp->ptname)) == 0)
					found = PTF_NMUSER;
			}
