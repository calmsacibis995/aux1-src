...
#define _AC_NO_MAIN "@(#) Copyright (c) 1987 Apple Computer, Inc., 1980-87 The Regents of the University of California, All Rights Reserved.  {Apple version 1.4 87/11/13 13:52:10}"
...
#define	snd (&so->so_snd)
			...
			if (unp->unp_addr)
				...
				from = &sun_noname;
			if (rights)
			    (void)sbappendaddr(rcv, from, m, rights,
				    so2->so_proto->pr_flags & PR_RIGHTS);
			else
				sbappend(rcv, m);
			snd->sb_mbmax -=
...
restart:
	...
	do {
		for (fp = file; fp < (struct file *)v.ve_file; fp++) {
			if (fp->f_count == 0 || fp->f_data == NULL)
				continue;
			if (fp->f_flag & FDEFER) {
				fp->f_flag &= ~FDEFER;
				unp_defer--;
...
#ifdef STANFORD
/*
 *  Hook for file sockets.
 */
uipc_fileconn(vp, fp, mode)
	...
	int mode;
{
...
#ifdef notdef
	/*
	 *  File must have been opened R/W.
	 */
	...
	unp2->unp_conn = unp;
	unp->unp_conn = unp2;
	unp2->unp_addr = m_get(M_WAIT, MT_SONAME);
	unp2->unp_addr->m_next = 0;
	unp2->unp_addr->m_len = 0;
	soisconnected(so2);
	soisconnected(so);
