#ifndef lint	/* .../appletalk/fwd/streams/stream.c */
#define _AC_NAME stream_c
#define _AC_NO_MAIN "@(#) Copyright (c) 1985-87 UniSoft Corporation, All Rights Reserved.  {Apple version 1.2 87/11/11 21:05:20}"
#include <apple_notice.h>

#ifdef _AC_HISTORY
  static char *sccsid = "@(#)Copyright Apple Computer 1987\tVersion 1.2 of stream.c on 87/11/11 21:05:20";
#endif		/* _AC_HISTORY */
#endif		/* lint */
...
register size;
{
	...
	if (!(bp = mbfreelist)) {
		 ...
		 */
		/*
		 * UNISOFT corrected, do not drop chain
		 */
...
queuerun()
{
	...
	while (q = qhead) {
		...
		TRACE(T_stream, ("queuerun: q = 0x%x, name = %s\n",
				q, q->q_qinfo->qi_minfo->mi_idname));
